 ...
 *
 * One journal per surah per member. Saves itself shortly after typing stops,
 * and again if the page is left with unsaved text, so a reflection is never
 * lost to a forgotten button. Row Level Security keeps each journal private
 * to its author in the database itself.
 */
export default function SurahNotes({
  ...
  const save = useCallback(async () => {
    ...
    setStatus("saving");
    const { error } = await supabase.from("quran_notes").upsert(
      { user_id: userId, surah_no: surah, body: text },
      { onConflict: "user_id,surah_no" },
    );
    if (error) {
      setStatus("error");
      return;
